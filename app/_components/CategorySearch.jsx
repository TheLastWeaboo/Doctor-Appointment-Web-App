...
function CategorySearch() {
    ...
    const [categoryList, setCategoryList] = useState([]); // Initialize as an empty array
    ...
    const getCategoryList = () => {
        GlobalApi.getCategory().then(resp => {
            ...
            setCategoryList(resp.data.data || []); // Ensure it is an array
        }).catch(error => {
            console.error("Error fetching categories: ", error);
            setCategoryList([]); // Set to an empty array on error
        });
    }
    ...
    return (
        ...
        text-4xl'>Search <span className='text-primary'>Doctors</span></h2>
